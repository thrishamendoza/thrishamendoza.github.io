...
audioElement.addEventListener("ended", () => {
  document.getElementById("audioLoopOverlay").style.display = "block";
  playButton.style.backgroundImage = "url('./icons/play.svg')";
});

audioElement.addEventListener("paused", () => {
  document.getElementById("audioLoopOverlay").style.display = "none";
});
...
// there is already an element appropriately placed as a signifier, the <img> with the id of audioPlayOverlay however its CSS is currently
...
/* MUTE/UNMUTE*/
//using the playpause function as a guide, I incorporated the .muted element where its default setting is set to false.  
function muteUnmute(){
  if (audioElement.muted) {
    audioElement.muted = false;
    muteButton.style.backgroundImage = "url('./icons/mute.svg')";
//to ensure that clicking the icon once more unmutes the audio.
  } else {
...
function disableMute(){
  audioElement.muted = false;
  unmuteButton.style.backgroundImage = "url('./icons/mute.svg')";
}

muteButton.addEventListener('click', enableMute);
unmuteButton.addEventListener('click', disableMute); */
...
/*REWIND/FAST FORWARD*/
/*using currentTime to make fast forward and rewinding functional in 5 second increments. I had been stuck on this for a bit as instead of inserting 'audioElement' id, I had typed in the rewindButton and forwardButton id's which wasn't working. */
...
forwardButton.addEventListener('click', fastForward);
